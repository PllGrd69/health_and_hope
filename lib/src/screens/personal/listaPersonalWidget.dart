...
class ListaPersonal extends StatelessWidget {
  const ListaPersonal({Key? key}) : super(key: key);
  @override
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<PersonalBloc, PersonalState>(
        builder: (context, state) {
          if (state.listPersonalPag!=null) return _ListaParticipante(participantes: state.listPersonalPag!);
          else if (state.listPersonalPag==null) return Center(child: CircularProgressIndicator());
          return Container();
        }
    );
  }
...
class _ListaParticipante extends StatefulWidget {
  final PersonalModelPagModel participantes;
...
class __ListaParticipanteState extends State<_ListaParticipante> {
  ...
  _scrollListener() {
    if (_controller.position.maxScrollExtent == _controller.offset) {
      ...
      if (results < count) {
        BlocProvider.of<PersonalBloc>(context, listen: false).add(
          ListaPersonalPagEvent(
...
class _ItemListParticipante extends StatelessWidget {
  ...
  final PersonalModel partcipante;
        ...
        children: [
          SlidableAction(
            onPressed: (BuildContext context) {
              BlocProvider.of<PersonalBloc>(context).add(
                  EliminarPersonalPagEvent(idPersonal: partcipante.id!)
              );
            },
      ...
      child: ListTile(
        leading: CircleAvatar(
          ...
          child: CircleImageAvatarWidget(
            imageUrl: partcipante.personal!.avatar,
            radio: responsive.ip(22),
            padding: responsive.ip(0.5),
          ),
        ),
        title: Text(
          '${partcipante.personal!.email}',
